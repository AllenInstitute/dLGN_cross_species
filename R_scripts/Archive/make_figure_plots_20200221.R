...
########################
## Collect all metadata to generate palettes
########################

load("macaque_allcl.rda")
anno_mac <- allcl
anno_mac$species <- "Macaque"
cl.df_mac <- anno_mac %>% 
				group_by(cluster_id,cluster_label, cluster_color, species) %>% 
				summarise(n=n()) 

load("human_allcl.rda")
anno_hum <- allcl
anno_hum$species <- "Human"
cl.df_hum <- anno_hum %>% 
				group_by(cluster_id,cluster_label, cluster_color, species) %>% 
				summarise(n=n()) 
...
anno_mo <- allcl
anno_mo$species <- "Mouse"
cl.df_mo <- anno_mo %>% 
				group_by(cluster_id,cluster_label, cluster_color, species) %>% 
				summarise(n=n()) 

cl.df <- rbind(cl.df_mac,cl.df_hum, cl.df_mo)
anno <- bind_rows(anno_mac,anno_hum, anno_mo)

write.csv(cl.df, file="cl.df.csv")

save(anno, cl.df, file="summ.all.cl.rda")
...
plot <- group_violin_plot(sampdat,allcl,
                    ...
                    genes=c("SNAP25","GAD2","SLC17A6","DLX1","NPY","TNFAIP8L3",
                    	...
                    	#"G0S2",
                    	"PENK","SGCD","DAB2","LVRN",
                    	"CASQ2","IL15RA","FOXP2","EBF1","FGF11","GRIK3",
                    	"PLPP4","RUFY4","PTPN20","GPR37","KCNJ2", "ROBO2", "ROBO3"),
                    log_scale=F,
                    label_height=15,
                    max_width=25)
...
dcol <- droplevels(setNames(donor_palette$donor_roi_color[donor_palette$species=="Macaque"], donor_palette$donor_roi[donor_palette$species=="Macaque"]))
