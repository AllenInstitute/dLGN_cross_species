...
if (spec=="macaque") {
  ...
  for (nam in names(subcells)) {
    ...
    m.overall <- ScaleData(object =m.overall, verbose = FALSE,vars.to.regress = "donor")
...
if (spec=="human") {
  ...
  allcl$cluster_label=gsub("GABA1_0","GABA3",allcl$cluster_label)
  allcl$cluster_label=gsub("GABA1_1|GABA1_2","GABA4",allcl$cluster_label)
  allcl$cluster_label=gsub("GABA2_0|GABA2_1","GABA1",allcl$cluster_label)
  allcl$cluster_label=gsub("GABA2_2","GABA2",allcl$cluster_label)
  table(allcl$cluster_label)
  ...
  save(allcl,file="human_allcl.rda")
  
  sampdat=data.frame(sample_name=allcl$sample_name,t(as.matrix(datlist[[spec]][,allcl$sample_name])))
  pdf("human_violin_plot.pdf",height=10,width=3)
  group_violin_plot(sampdat,allcl,grouping="cluster",
  ...
  save(plot_objects,file="human_plotobjects.rda")
  ...
  pdf("human_umap_plot.pdf",useDingbats=F)
  temp=plot_objects[["all"]]
  dimval=26
  ...
  propvals=sweep(propvals,1,rowSums(propvals),"/")*100
  pdf("human_proportions.pdf",useDingbats=F,width=10,height=6)
  barplot(t(propvals[,c(2,3,1)]),col=c("blue","red","grey"),ylab="Fraction of nuclei")
  dev.off()
  
  ###hierarchical plot###
  pdf("human_dendrogram.pdf")
  temp=plot_objects[["all"]]
